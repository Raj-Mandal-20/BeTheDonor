    ...
    <div className="bg-[#051a39] flex items-start p-4 justify-center min-h-screen">
      <div className="bg-white w-2/3 p-6 rounded-lg shadow-lg ">
        ...
        <div className="mb-4">
          ...
          <p className="text-gray-900">{profile.city}, {profile.district}, {profile.state}, {profile.pin} </p>
        </div>
        <div>
        ...
        {
            history?.length > 0 && history?.map((request, index) => {
                return <RequestHistory key={index} request={request} HOST={props.HOST} />
            })
        }
        {
            profile.requests?.length === 0 && <div className="text-black text-2xl]">No Request History</div>
        }
